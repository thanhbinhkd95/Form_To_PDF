...
async function uploadViaFunction(pdfBlob, filename) {
  ...
  const projectId = import.meta.env.VITE_FIREBASE_PROJECT_ID || 'form-to-pdf-52d69'
  ...
  try {
    ...
    console.log('Local proxy error, falling back to direct Cloud Function:', error.message)
  }
  ...
  try {
    ...
    console.log('Upload successful via direct Cloud Function:', data.downloadURL)
    return data.downloadURL
  } catch (error) {
