...
import { ConfirmDialog } from "../shared/SharedDialog.jsx";
import {
  Label,
  ErrorMessage,
  Input,
  ...
  Section,
  Row,
} from "../shared/FormComponents.jsx";
...
export default function FormInput({ initialValues }) {
  const { updateForm, submitForm, validationErrors, clearValidationErrors } =
    useForm();
    ...
    <form className="grid-1 form-print-wrapper" onSubmit={handleSubmit}>
      <div className="form-header">
        ...
        <div className="form-instruction">
          <p className="form-instruction-jp">
            {formText.instruction.split(".")[0]}.
          </p>
          <p className="form-instruction-en">
            {formText.instruction.split(".")[1]}
          </p>
        </div>
      </div>
      ...
      <Row cols={2}>
        <div>
          ...
          <Input
            value={d.lastNameRomaji}
            onChange={(v) => updateForm({ lastNameRomaji: v })}
          />
        ...
        <div>
          ...
          <Input
            value={d.firstNameRomaji}
            onChange={(v) => updateForm({ firstNameRomaji: v })}
          />
        ...
        <div>
          ...
          <Input
            value={d.lastNameKanji}
            onChange={(v) => updateForm({ lastNameKanji: v })}
          />
        </div>
        <div>
          <Label>名(漢字)/First Name (Kanji)</Label>
          <Input
            value={d.firstNameKanji}
            onChange={(v) => updateForm({ firstNameKanji: v })}
          />
        </div>
      </Row>
      <Row cols={3}>
      ...
      <Row cols={1}>
        <div>
          ...
          <Textarea
            ...
            onChange={(v) => updateForm({ permanentAddress: v })}
          />
      ...
      <Row cols={1}>
        <div>
          ...
          <Textarea
            ...
            onChange={(v) => updateForm({ currentAddress: v })}
          />
      ...
      <Row cols={2}>
        <div>
          ...
          <Input
            ...
            placeholder="Country Code should be included"
          />
      ...
      <Row cols={3}>
        <div>
          ...
          <Input
            ...
            onChange={(v) => updateForm({ passportNumber: v })}
          />
        </div>
        <div>
          ...
          <Input
            ...
            onChange={(v) => updateForm({ passportIssueDate: v })}
          />
        </div>
        <div>
          <Label>発行地/Issue Place</Label>
          <Input
            value={d.passportIssuePlace}
            onChange={(v) => updateForm({ passportIssuePlace: v })}
          />
        </div>
      </Row>
      <Row cols={2}>
        <div>
          ...
          <Input
            type="date"
            value={d.passportExpirationDate}
            onChange={(v) => updateForm({ passportExpirationDate: v })}
          />
        </div>
        <div>
      ...
      <Row cols={3}>
        <div>
          <Label>Yes/No</Label>
          <YesNo
            value={d.coeHistory?.yesNo}
            onChange={(v) =>
              updateForm({ coeHistory: { ...d.coeHistory, yesNo: v } })
            }
          />
        </div>
        <div>
          ...
          <Input
            type="number"
            value={d.coeHistory?.count}
            onChange={(v) =>
              updateForm({ coeHistory: { ...d.coeHistory, count: v } })
            }
          />
        </div>
        <div>
          ...
          <Input
            type="number"
            value={d.coeHistory?.deniedCount}
            onChange={(v) =>
              updateForm({ coeHistory: { ...d.coeHistory, deniedCount: v } })
            }
          />
        </div>
      </Row>
      ...
      <Row cols={3}>
        <div>
          <Label>Yes/No</Label>
          <YesNo
            value={d.visits?.yesNo}
            onChange={(v) => updateForm({ visits: { ...d.visits, yesNo: v } })}
          />
        </div>
        <div>
          <Label>回数/How many times</Label>
          <Input
            type="number"
            value={d.visits?.count}
            onChange={(v) => updateForm({ visits: { ...d.visits, count: v } })}
          />
        </div>
        <div>
          <Label>直近の出入国/Most Recent Entry/Exit</Label>
          <Input
            value={d.visits?.recent}
            onChange={(v) => updateForm({ visits: { ...d.visits, recent: v } })}
      ...
      <Row cols={3}>
        <div>
          ...
          <Input
            value={d.lastSchoolSummary}
            onChange={(v) => updateForm({ lastSchoolSummary: v })}
          />
        </div>
        <div>
          <Label>区分/Category</Label>
          <Input
            value={d.lastSchoolCategory}
            onChange={(v) => updateForm({ lastSchoolCategory: v })}
          />
        </div>
        <div>
          ...
          <Input
            type="number"
            value={d.yearsFromElementary}
            onChange={(v) => updateForm({ yearsFromElementary: v })}
          />
        </div>
      </Row>
      ...
      <div style={{ marginBottom: 8 }}>
        <Label>Yes/No</Label>
        <YesNo
          value={d.employmentYesNo}
          onChange={(v) => updateForm({ employmentYesNo: v })}
        />
      </div>
      {d.employmentYesNo === "Yes" && (
        <Table
          ...
          rows={
            d.employment && d.employment.length
              ? d.employment
              : [
                  {
                    label: "Company 1",
      ...
      <Row cols={2}>
        <div>
          <Label>
            Have you ever learnt Japanese in the language school? Yes/No
          </Label>
          <YesNo
            value={d.hasStudiedAtLanguageSchool}
            onChange={(v) => updateForm({ hasStudiedAtLanguageSchool: v })}
          />
        </div>
        <div>
          ...
          <Input
            type="number"
            value={d.jpLearningHours}
            onChange={(v) => updateForm({ jpLearningHours: v })}
          />
        </div>
      </Row>
      {d.hasStudiedAtLanguageSchool === "Yes" && (
        <Table
          columns={jpSchoolCols}
          rows={
            d.jpSchools && d.jpSchools.length
              ? d.jpSchools
              : [
      ...
      <Table
        columns={proficiencyCols}
        rows={
          d.proficiency && d.proficiency.length
            ? d.proficiency
            : [
                { label: "JLPT" },
                { label: "J.Test" },
      ...
      <Row cols={1}>
        <div>
          <Label>
            その他の場合、テスト名を記入ください/Please put test name if
            applicant takes "Others" above
          </Label>
          <Input
            value={d.otherProficiencyNote}
            onChange={(v) => updateForm({ otherProficiencyNote: v })}
          />
        </div>
      </Row>
      ...
      <Row cols={2}>
        <div>
          ...
          <Input
            value={d.sponsor?.fullName}
            onChange={(v) =>
              updateForm({ sponsor: { ...d.sponsor, fullName: v } })
            }
          />
        </div>
        <div>
          ...
          <Input
            value={d.sponsor?.relationship}
            onChange={(v) =>
              updateForm({ sponsor: { ...d.sponsor, relationship: v } })
            }
          />
        </div>
      </Row>
      <Row cols={2}>
        <div>
          <Label>現住所/Current Address</Label>
          <Textarea
            value={d.sponsor?.currentAddress}
            onChange={(v) =>
              updateForm({ sponsor: { ...d.sponsor, currentAddress: v } })
            }
          />
        </div>
        <div>
          ...
          <Input
            type="email"
            value={d.sponsor?.email}
            onChange={(v) =>
              updateForm({ sponsor: { ...d.sponsor, email: v } })
            }
          />
        </div>
      </Row>
      <Row cols={3}>
        <div>
          <Label>電話番号/Phone</Label>
          <Input
            value={d.sponsor?.phone}
            onChange={(v) =>
              updateForm({ sponsor: { ...d.sponsor, phone: v } })
            }
          />
        </div>
        <div>
          ...
          <Input
            value={d.sponsor?.company}
            onChange={(v) =>
              updateForm({ sponsor: { ...d.sponsor, company: v } })
            }
          />
        </div>
        <div>
          ...
          <Input
            value={d.sponsor?.position}
            onChange={(v) =>
              updateForm({ sponsor: { ...d.sponsor, position: v } })
            }
          />
        </div>
      </Row>
      <Row cols={3}>
        <div>
          ...
          <Input
            value={d.sponsor?.workAddress}
            onChange={(v) =>
              updateForm({ sponsor: { ...d.sponsor, workAddress: v } })
            }
          />
        </div>
        <div>
          ...
          <Input
            value={d.sponsor?.workPhone}
            onChange={(v) =>
              updateForm({ sponsor: { ...d.sponsor, workPhone: v } })
            }
          />
        </div>
        <div>
          ...
          <Input
            type="number"
            value={d.sponsor?.annualIncomeJpy}
            onChange={(v) =>
              updateForm({ sponsor: { ...d.sponsor, annualIncomeJpy: v } })
            }
          />
        </div>
      </Row>
      <Row cols={1}>
        <div>
          ...
          <Input
            type="number"
            value={d.sponsor?.exchangeRate}
            onChange={(v) =>
              updateForm({ sponsor: { ...d.sponsor, exchangeRate: v } })
            }
            placeholder="1USD = xxx JPY"
          />
        </div>
      </Row>
      ...
      <div style={{ marginBottom: 8 }}>
        <Label>
          日本に家族はいますか？/Do you have any family members in Japan?
        </Label>
        <YesNo
          value={d.familyInJapanYesNo}
          onChange={(v) => updateForm({ familyInJapanYesNo: v })}
        />
      </div>
      {d.familyInJapanYesNo === "Yes" && (
        <Table
          columns={familyJpCols}
          rows={d.familyInJapan || []}
          onChange={(rows) => updateForm({ familyInJapan: rows })}
          onAdd={() =>
            updateForm({ familyInJapan: [...(d.familyInJapan || []), {}] })
          }
          addLabel="Add Member"
        />
      )}
      ...
      <Row cols={2}>
        <div>
          ...
          <Input
            value={d.schoolType}
            onChange={(v) => updateForm({ schoolType: v })}
          />
        </div>
        <div>
      ...
      <Row cols={3}>
        ...
        <div>
          ...
          <Input
            value={d.desiredJob}
            onChange={(v) => updateForm({ desiredJob: v })}
          />
        </div>
        <div>
          <Label>帰国予定/Return Home (YYYY/MM)</Label>
          <Input
            value={d.returnHomeYyyyMm}
            onChange={(v) => updateForm({ returnHomeYyyyMm: v })}
            placeholder="YYYY/MM"
          />
        </div>
      </Row>
      ...
      <Row cols={1}>
        <div>
          ...
          <Textarea
            value={d.reasonsForApplying}
            onChange={(v) => updateForm({ reasonsForApplying: v })}
            placeholder="Please describe your reasons for applying..."
          />
        </div>
      </Row>
      ...
      <div style={{ marginTop: 16, textAlign: "center" }}>
        ...
        >
          {imageUrl ? (
            ...
            >
              <div
                style={{ fontSize: "24px", marginBottom: "8px", opacity: 0.7 }}
              >
                📷
              </div>
              <div
              ...
              <div
                style={{ fontSize: "10px", marginBottom: "4px", opacity: 0.8 }}
              >
      ...
      <Row cols={3}>
        {[
          ...
          const currentAttachment = state.attachments?.find(
            (att) => att.key === key
          );
          ...
          const handleFileChange = async (e) => {
            ...
            if (file) {
              try {
                ...
                alert(
                  "ファイル処理エラー / File processing error. Please try again."
                );
              }
            }
          };
